...
for arch in "${arches[@]}"; do
  ...
  for registry in "${registries[@]}"; do
    ...
    if [[ ${SIMPLE_RELEASE:-false} == true ]]; then
      # Exact release version only — no floating tags (latest / major / minor).
...
if [[ ${DRY_RUN:-false} != true && ${SIMPLE_RELEASE:-false} != true ]]; then
  # Exact release version only. No floating tags like latest / major /
  # major.minor — each release has one definitive version.
  for registry in "${registries[@]}"; do
